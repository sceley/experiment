...
exports.addExp = async (req, res) => {
	try {
		...
		const expsCount = await new Promise((resolve, reject) => {
			...
			db.connection.query(sql, [body.name], (err, exps) => {
				if (err)
					reject(err);
				else
		...
		await new Promise((resolve, reject) => {
			...
			db.connection.query(sql, [body.name, body.address], (err) => {
				if (err)
					reject(err);
				else
					resolve();
		...
		for (let i = 1; i <= body.tablesCount; i++)
			await new Promise((resolve, reject) => {
				...
				db.connection.query(sql, [exp_id, i], err => {
					if (err)
						reject(err);
					else 
						resolve();
...
exports.showExps = async (req, res) => {
	try {
		const exps = await new Promise((resolve, reject) => {
			...
			db.connection.query(sql, (err, exps) => {
				if (err)
					reject(err);
				else
					resolve(exps);
...
exports.showRestExps = async (req, res) => {
	try {
		...
		const reserves = await new Promise((resolve, reject) => {
			...
			db.connection.query(sql, [body.date, body.end, body.start, body.end, body.start, body.start, body.end], (err, reserves) => {
				if (err)
					reject(err);
				else
					resolve(reserves);
		...
		const fault_tabs = await new Promise((resolve, reject) => {
			...
			db.connection.query(sql, [1], (err, tabs) => {
				if (err)
					reject(err);
				else
					resolve(tabs);
...
exports.editExp = async (req, res) => {
	try {
		...
		await new Promise((resolve, reject) => {
			...
			db.connection.query(sql, [body.name, body.address, body.id], err => {
				if (err)
					reject(err);
				else
					resolve();
		...
		await new Promise((resolve, reject) => {
			...
			db.connection.query(sql, [body.id], err => {
				if (err)
					reject(err);
				else
					resolve();
		...
		for (let i = 1; i <= body.tablesCount; i++) {
			await new Promise((resolve, reject) => {
				...
				db.connection.query(sql, [body.id, i], err => {
					if (err)
						reject(err);
					else
						resolve();
...
exports.monitorExp = async (req, res) => {
	try {
		...
		let tables = await new Promise((resolve, reject) => {
			...
			db.connection.query(sql, [id], (err, tables) => {
				if (err)
				reject(err);
				else
				resolve(tables);
