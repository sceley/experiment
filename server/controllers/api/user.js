...
exports.editInfo = async (req, res) => {
	...
	try {
		...
		await new Promise((resolve, reject) => {
			let sql = 'update User set account=?, sex=?, grade=?, major=?, name=?, mobile=?, active=? where id=?';
			db.query(sql, [body.SchoolNumber, body.Sex, body.Grade, body.Major, body.Name, body.Mobile, true, id], (err) => {
				if (err)
					reject(err);
				else 
					resolve();
